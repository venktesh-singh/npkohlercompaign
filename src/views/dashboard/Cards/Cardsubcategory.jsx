...
function CardSale() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:4001/api/v1/subcategories/get/counts');
  ...
  //console.log("Product sale:", sale); // Use optional chaining to avoid errors

  return (
    <GradientCard>
      <Link style={{color:'#fff'}} to={`/subcategory/list`}><Typography variant="h4" style={{color:'#fff'}}>Subcategory <ArrowForwardIcon color="white"/></Typography></Link>
      <Typography variant="h4" style={{color:'#fff'}}>
        ...
        {sale ? sale.subcategoryCount : 'Loading...'}
      </Typography>
    </GradientCard>
  );
}
