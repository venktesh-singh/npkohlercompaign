...
function PopFormList() {
    ...
    const fetchData = async () => {
        try {
            const response = await fetch('http://localhost:4001/api/v1/commonforms/',{
                method: 'GET',
            });
            if (!response.ok) {
            ...
            setAppointment(data.commonForm);
        } catch (err) {
    ...
    const handleDelete = async (id) => {
        try {
            const response = await fetch(`http://localhost:4001/api/v1/commonforms/${id}`, {
                method: 'DELETE',
            });
                                ...
                                {filteredAppointments && filteredAppointments.length > 0 ? (
                                    filteredAppointments.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((appointment, index) => (
                                        <tr key={appointment._id}>
                                            ...
                                            <td>
                                                <Link to={`/popform/detail`} state={{ appointment }} style={{ padding: 10 }}>
